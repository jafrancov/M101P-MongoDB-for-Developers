...
class BlogPostDAO:
    ...
    def add_comment(self, permalink, name, email, body):
        ...
        try:
            # XXX HW 3.3 Work here to add the comment to the designated post. When done, modify the line below
            # to return the number of documents updated by your modification, rather than just -1.
            post = self.posts.update({'permalink': permalink}, {'$push': {'comments': comment}})

            return len(post['comments'])  # Change this to return the number of documents updated by the code for HW 3.3
